...
function WidgetFormContent() {
  ...
  // Autocomplete states
  const [countySuggestions, setCountySuggestions] = useState<any[]>([]);
  ...
  async function searchCityAutocomplete(text: string, countyFilter?: string) {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Filtrează după județ dacă este setat
  ...
  async function searchAddressAutocomplete(text: string, cityFilter?: string, countyFilter?: string) {
    ...
    try {
      // Construiește textul de căutare cu context
      let searchText = text;
      if (cityFilter) {
  ...
  function selectCounty(suggestion: any) {
    setCounty(suggestion.county || suggestion.name);
  ...
  function selectCity(suggestion: any) {
    setCity(suggestion.city || suggestion.name);
    setSelectedPostalCode(suggestion.postcode || "");
    setCitySuggestions([]);
    setShowCitySuggestions(false);
  }
  ...
  function selectAddress(suggestion: any) {
    const fullAddress = suggestion.address_line1 || suggestion.formatted || suggestion.name;
    setAddress(fullAddress);
    setSelectedPostalCode(suggestion.postcode || "");
    // Auto-fill city and county if not set
                  ...
                  {showCountySuggestions && countySuggestions.length > 0 && (
                    <div className="absolute z-10 w-full mt-1 bg-white border border-zinc-300 rounded-lg shadow-lg max-h-60 overflow-auto">
                      {countySuggestions.map((suggestion, idx) => (
                        ...
                        >
                          <div className="font-medium">{suggestion.county || suggestion.name}</div>
                          {suggestion.state && (
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-zinc-300 rounded-lg shadow-lg max-h-60 overflow-auto">
                      {citySuggestions.map((suggestion, idx) => (
                        <button
                          ...
                          className="w-full text-left px-3 py-2 hover:bg-zinc-50 text-sm text-zinc-900 border-b border-zinc-100 last:border-b-0"
                        >
                          <div className="font-medium">{suggestion.city || suggestion.name}</div>
                          {suggestion.postcode && (
