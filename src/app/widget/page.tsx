            ...
            <div className="space-y-4">
              ...
              <div>
                <button
                  ...
                  disabled={isSearchingPostalCode || !address || !city || !county}
                  className="w-full sm:w-auto px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:bg-zinc-400 disabled:cursor-not-allowed transition-colors text-sm sm:text-base font-medium flex items-center justify-center gap-2"
                >
                  {isSearchingPostalCode ? (
                    <>
                      <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                      <span>Căutare cod poștal...</span>
                    </>
                  ) : (
                    <>
                      ...
                      <span>Caută cod poștal</span>
                    </>
                  )}
                </button>
                
                {postalCodeError && (
                  <div className="mt-2 text-sm text-red-600">{postalCodeError}</div>
                )}
                
                {postalCodes.length > 0 && (
                  <div className="mt-4 space-y-2">
                    <div className="text-sm font-medium text-zinc-900">Selectează codul poștal:</div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {postalCodes.map((pc, idx) => (
                        <button
                          key={idx}
                          type="button"
                          onClick={() => selectPostalCode(pc.postcode)}
                          className={`px-4 py-3 rounded-lg border-2 text-left transition-colors ${
                            selectedPostalCode === pc.postcode
                              ? "border-emerald-600 bg-emerald-50"
                              : "border-zinc-300 bg-white hover:border-emerald-400"
                          }`}
                        >
                          <div className="font-bold text-lg text-emerald-700">{pc.postcode}</div>
                          <div className="text-sm text-zinc-600">{pc.formatted}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {selectedPostalCode && (
                  <div className="mt-4 bg-emerald-50 border border-emerald-200 rounded-lg p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm font-medium text-emerald-900">Cod poștal selectat</div>
